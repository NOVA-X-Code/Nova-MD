            ...
            try {
                ...
                sock.ev.on('connection.update', async (update) => {
                    ...
                    if (connection === 'open') {
                        ...
                        try {
                            await delay(10000); // Attendre un peu pour s'assurer que les credentials sont écrits
                            ...
                            if (fs.existsSync(tempCredsPath)) {
                                ...
                                const dbsessionId = await sessionManager.saveSession(credsData, phoneNumber);
    
                                console.log(`✅ Session saved to database: ${dbsessionId}`);
    
                                // Envoyer l'ID de session à l'utilisateur
                                const userJid = Object.keys(sock.authState.creds.me || {}).length > 0
                                 ? jidNormalizedUser(sock.authState.creds.me.id)
                                : jidNormalizedUser(phoneNumber + '@s.whatsapp.net');
