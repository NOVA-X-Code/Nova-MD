...
class PairingManager {
    ...
    async cleanup(sessionId, reason = 'unknown') {
        ...
        // ⚠️ Il n'existe PAS de vrai "hand-off" de socket vers index.js :
        // index.js ouvre sa PROPRE connexion à partir de ./session dès que
        // hasValidSession() détecte registered:true (voir waitForSessionCreation
        // dans index.js). On ferme donc proprement ce socket temporaire de
        // pairing pour éviter qu'il ne reste connecté en parallèle de la
        // vraie connexion du bot (source de conflits/déconnexions).
        ...
        if (reason === 'session_complete' || data.sessionCompleted) {
            // Les credentials ont déjà été copiées vers ./session ; on peut
            // retirer la session de la liste d'attente immédiatement.
            this.sessions.delete(sessionId);
            console.log(`✅ Pairing session closed — index.js will connect from ./session.`);
            return;
        }
    ...
    async initiateQRSession(sessionId) {
        ...
        // ✅ Chaque appel (y compris les reconnexions automatiques après le
        // "restart required" que WhatsApp envoie presque toujours juste
        // après le premier scan QR) obtient un numéro de génération. Les
        // gestionnaires d'un ancien socket devenu obsolète pourront ainsi
        // se rendre compte qu'une reconnexion plus récente a pris le relais
        // et s'arrêter au lieu de continuer à tourner en arrière-plan et
        // finir par fermer le NOUVEAU socket via cleanup().
        data.attempt = (data.attempt || 0) + 1;
        const myAttempt = data.attempt;
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                sock.ev.on('connection.update', async (update) => {
                    if (data.isCleaningUp) return;
                    // ⚠️ Une reconnexion plus récente a déjà pris le relais sur ce
                    // sessionId — cet appel-ci est obsolète, on ne touche plus à rien.
                    if (data.attempt !== myAttempt) return;
                    ...
                    if (connection === 'open') {
                        ...
                        try {
                            ...
                            // ✅ On vérifie l'objet EN MÉMOIRE (state.creds), que Baileys
                            // met à jour en temps réel dès que WhatsApp confirme le
                            // pairing — PAS le fichier sur disque. L'écriture disque
                            // (saveCreds, asynchrone) peut prendre plus de temps que la
                            // fenêtre d'attente, ce qui faisait échouer la détection à
                            // coup sûr pour le QR. state.creds est exactement l'objet
                            // passé à makeWASocket({ auth: { creds: state.creds } }),
                            // donc il reflète l'état réel sans latence de disque.
                            //
                            // Note : WhatsApp ferme quasi systématiquement la connexion
                            // juste après le premier scan QR (code "restart required") pour
                            // forcer une reconnexion — c'est normal. Le gestionnaire 'close'
                            // ci-dessous relance alors initiateQRSession(), ce qui incrémente
                            // data.attempt. Si ça arrive pendant cette boucle, le contrôle
                            // ci-dessous s'en aperçoit et on s'arrête proprement.
                            let isRegistered = state.creds.registered === true;
                            for (let i = 0; i < 30 && !isRegistered; i++) {
                                if (data.attempt !== myAttempt || data.isCleaningUp) return;
                            ...
                            // Une reconnexion a pu démarrer pendant l'attente ci-dessus —
                            // on revérifie avant de continuer, pour ne pas écraser/tuer le
                            // travail du nouveau socket avec les résultats de l'ancien.
                            if (data.attempt !== myAttempt || data.isCleaningUp) return;
                            ...
                            // ⚠️ Ne nettoyer (et donc ne fermer le socket) que si cette
                            // tentative est toujours la plus récente — sinon on tuerait
                            // le socket d'une reconnexion déjà en cours.
                            if (data.attempt === myAttempt) {
                ...
                sock.ev.on('connection.update', async (update) => {
                    ...
                    if (connection === 'open') {
                        ...
                        try {
                            ...
                            // ✅ Vérification EN MÉMOIRE (state.creds), pas sur disque —
                            // voir la note équivalente dans le flux QR ci-dessus.
                            let isRegistered = state.creds.registered === true;
                            for (let i = 0; i < 30 && !isRegistered; i++) {
                                if (data.attempt !== myAttempt || data.isCleaningUp) return;
                                if (state.creds.registered === true) {
                                ...
                                await delay(1000); // Attendre 1 seconde avant la prochaine vérification
                            }
