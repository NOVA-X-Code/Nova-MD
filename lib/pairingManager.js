...
function randomNovaId(len = 6, numLen = 4) {
                    ...
                    if (connection === 'open') {
                        ...
                        try {
                            ...
                            if (fs.existsSync(tempCredsPath)) {
                                ...
                                const id = randomNovaId();
                                const megaLink = await megaUpload(await fs.readFile(tempCredsPath), `${id}.json`);
                    ...
                    if (connection === 'open') {
                        ...
                        try {
                            ...
                            if (fs.existsSync(tempCredsPath)) {
                                ...
                                const megaLink = await megaUpload(await fs.readFile(tempCredsPath), `${id}.json`);
                                const megaSessionId = megaLink.replace('https://mega.nz/file/', '');
                                const userJid = jidNormalizedUser(data.phoneNumber + '@s.whatsapp.net');
                                await delay(2000);
                                const msg = await sock.sendMessage(userJid, { text: megaSessionId });
                                await sock.sendMessage(userJid, { text: MESSAGE, quoted: msg });
                                await delay(1000);
