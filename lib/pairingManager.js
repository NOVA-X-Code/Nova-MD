        ...
        sock.ev.on("connection.update", async (update) => {
          ...
          if (connection === "open") {
            ...
            try {
              ...
              // ✅ 1. Définir les chemins une seule fois
              ...
              if (fs.existsSync(tempCredsPath)) {
                try {
                  // ✅ 3. Lire et parser une seule fois
                  const rawData = await fs.readFile(tempCredsPath, "utf-8");
                  const credsData = JSON.parse(rawData);

                  // ✅ 4. Vérifier que la session est bien enregistrée
                  if (credsData.registered !== true) {
                    console.log(
                      "⚠️ Credentials not yet registered, waiting...",
                    );
                    // Option: attendre ou logger l'erreur
                    return; 
                  }

                  // ✅ 5. Marquer la session comme complétée
                  data.sessionCompleted = true;

                  // ✅ 6. Créer le dossier principal s'il n'existe pas
                  ...
                  // ✅ 8. Extraire le numéro de téléphone
                  ...
                  // ✅ 10. Envoyer l'ID de session à l'utilisateur
                  ...
                  // Gérer l'erreur (fichier corrompu, permissions, etc.)
                }
              } else {
                console.log("⚠️ Credentials file not found at:", tempCredsPath);
              }
            } catch (err) {
          ...
          if (connection === "open") {
            ...
            try {
              ...
              for (let attempt = 0; attempt < 10; attempt++) {
                // Limite à 10 tentatives) {
                if (fs.existsSync(tempCredsPath)) {
                  const credsData = JSON.parse(
                    await fs.readFile(tempCredsPath, "utf-8"),
                  );
              ...
              if (!isRegistered) {
                console.log(
                  "⚠️ Validation trop longue, copie de sauvegarde lancée par sécurité.",
                );
              }
              ...
              if (fs.existsSync(tempCredsPath)) {
                ...
                const phoneNumber =
                  ...
                  null;
                const saveId = await sessionManager.saveSession(
                  credsData,
                  phoneNumber,
                );
