        ...
        sock.ev.on("connection.update", async (update) => {
          ...
          if (connection === "open") {
            ...
            try {
              ...
              // ✅ ATTENDRE QUE LA SESSION SOIT COMPLÈTEMENT ENREGISTRÉE
              ...
              while (attempts < maxAttempts && !registered) {
                ...
                try {
                  // Lire le fichier creds.json du dossier principal
                  const mainCredsPath = path.join(mainSessionDir, "creds.json");
                  if (fs.existsSync(mainCredsPath)) {
                    const credsData = JSON.parse(
                      fs.readFileSync(mainCredsPath, "utf-8"),
                    );

                    if (credsData.registered === true) {
                      registered = true;
                      console.log(
                        `✅ Session registered successfully! (${attempts}s)`,
                      );
                ...
                if (attempts % 5 === 0) {
                  console.log(
                    `⏳ Still waiting for registration... (${attempts}s)`,
                  );
                }
              }
