...
export const smsg = (NovaXCode, m, store) => {
    if (!m)
        return m;
    ...
    if (m.key) {
        ...
        m.sender = NovaXCode.decodeJid(m.fromMe && NovaXCode.user.id || m.participant || m.key.participant || m.chat || '');
        if (m.isGroup)
            m.participant = NovaXCode.decodeJid(m.key.participant) || '';
    }
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.sender = NovaXCode.decodeJid(m.msg.contextInfo.participant);
            m.quoted.fromMe = m.quoted.sender === (NovaXCode.user && NovaXCode.user.id);
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.conversation || m.quoted.contentText || m.quoted.selectedDisplayText || m.quoted.title || '';
            m.quoted.mentionedJid = m.msg.contextInfo ? m.msg.contextInfo.mentionedJid : [];
            m.getQuotedObj = m.getQuotedMessage = async () => {
                if (!m.quoted.id)
                    return false;
                const q = await store.loadMessage(m.chat, m.quoted.id, NovaXCode);
                return smsg(NovaXCode, q, store);
            };
            ...
            m.quoted.delete = () => NovaXCode.sendMessage(m.quoted.chat, { delete: vM.key });
            m.quoted.copyNForward = (jid, forceForward = false, options = {}) => NovaXCode.copyNForward(jid, vM, forceForward, options);
            m.quoted.download = () => NovaXCode.downloadMediaMessage(m.quoted);
        }
    }
    if (m.msg?.url)
        m.download = () => NovaXCode.downloadMediaMessage(m.msg);
    m.text = m.msg?.text || m.msg?.caption || m.message?.conversation || m.msg?.contentText || m.msg?.selectedDisplayText || m.msg?.title || '';
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text)
        ? NovaXCode.sendMedia(chatId, text, 'file', '', m, { ...options })
        : NovaXCode.sendText(chatId, text, m, { ...options });
    m.copy = () => smsg(NovaXCode, M.fromObject(M.toObject(m)), store);
    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => NovaXCode.copyNForward(jid, m, forceForward, options);
    return m;
};
