...
// Timeout du serveur (30 minutes)
server.timeout = 30 * 60 * 1000;
server.keepAliveTimeout = 30 * 60 * 1000;
server.headersTimeout = 31 * 60 * 1000;

// Variables pour le keep-alive
...
// ============================================================
// PAIRING TIMEOUT MANAGEMENT
// ============================================================

function startPairingTimeout() {
    if (pairingTimeout) clearTimeout(pairingTimeout);
    
    // Si pas de session après 45 minutes, redémarrer
    pairingTimeout = setTimeout(() => {
        if (!hasValidSession()) {
            console.log('⏰ Pairing timeout: No session created after 45 minutes');
            console.log('🔄 Restarting server to refresh pairing interface...');
            process.exit(1);
        }
    }, 45 * 60 * 1000);
}
...
// Démarrer le timeout au lancement
if (!hasValidSession()) {
    setTimeout(startPairingTimeout, 10000);
}
...
function startKeepAlive() {
    ...
    keepAliveInterval = setInterval(() => {
        ...
        if (timeSinceLastRequest > 5 * 60 * 1000) {
            console.log('💓 Keep-alive ping (no activity for 5+ minutes)');
            
            // Ping interne pour garder le serveur actif
...
app.get('/pairing', (req, res) => {
    ...
    try {
        ...
        res.status(500).send('Error loading pairing page');
    }
});
        ...
        if (fs.existsSync(credsPath)) {
            try {
                ...
                if (creds.registered === true) {
                    // Arrêter le timeout car session créée
                    if (pairingTimeout) {
                        clearTimeout(pairingTimeout);
                        pairingTimeout = null;
                    }
...
// ✅ Version avec vérification en temps réel du statut de session
app.get('/', (req, res) => {
    resetPairingTimeout();
    ...
    const sessionConnected = hasValidSession();
    const statusClass = sessionConnected ? 'connected' : 'waiting';
    const statusText = sessionConnected ? '✅ Connected to WhatsApp' : '⏳ Waiting for WhatsApp connection...';
    const statusColor = sessionConnected ? '#25d366' : '#ffc107';
    ...
    <head>
        ...
        <style>
            ...
            .status-msg {
                ...
                border-radius: 8px;
                font-size: 0.8rem;
                background: rgba(${sessionConnected ? '37, 211, 102' : '255, 193, 7'}, 0.1);
                color: ${statusColor};
            }
            .status-msg.waiting {
    ...
    <body>
        <div class="container">
            ...
            <div class="status-msg ${statusClass}" id="statusMsg">
                ${statusText}
            </div>
        ...
        <script>
            ...
            function checkStatus() {
                fetch('/api/pairing/session/status')
                    .then(res => res.json())
                    .then(data => {
                        const msg = document.getElementById('statusMsg');
                        if (data.hasSession) {
                            // Vérifier si la session est vraiment enregistrée
                            fetch('/session/creds.json')
                                .then(r => r.json())
                                .then(creds => {
                                    if (creds.registered === true) {
                                        msg.className = 'status-msg connected';
                                        msg.textContent = '✅ NOVA-MD Connected';
...
// ============================================================
// ✅ ROUTE POUR SERVIR LE FICHIER CREDS.JSON (pour vérification)
// ============================================================

app.get('/session/creds.json', (req, res) => {
    try {
        const credsPath = path.join(process.cwd(), 'session', 'creds.json');
        if (!fs.existsSync(credsPath)) {
...
app.get('/health', (req, res) => {
    ...
    res.json({
        ...
        session: hasSession ? 'connected' : 'waiting',
        memory: {
