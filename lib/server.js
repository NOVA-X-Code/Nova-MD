...
/**
 * GET /api/pairing/view/qr - Vue QR code
 */
app.get('/api/pairing/view/qr', (req, res) => {
    const qrHtml = fs.readFileSync(path.join(VIEWS_DIR, 'qr-method.html'), 'utf8');
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(qrHtml);
});

/**
 * GET /api/pairing/view/pair - Vue Pairing Code
 */
app.get('/api/pairing/view/pair', (req, res) => {
    const pairHtml = fs.readFileSync(path.join(VIEWS_DIR, 'pair-method.html'), 'utf8');
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(pairHtml);
});
