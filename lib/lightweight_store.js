 ...
 *                     Developed By Nostra                                   *
 *                                                                           *
 *  🌐  GitHub   : https://github.com/Labokingfreesurf                       *
        ...
        adapters.mongo = {
            ...
            async getAllCounts() {
                try {
                    ...
                    const result = { isPublic: false, messageCount: {} }; // 🔒 PRIVÉ
                    docs.forEach(doc => {
                    ...
                    return { isPublic: false, messageCount: {} }; // 🔒 PRIVÉ
                }
            },
...
const store = {
    ...
    chats: {},
    messageCount: {},
    isPublic: false, // 🔒 PRIVÉ PAR DÉFAUT
    botMode: 'private', // 🔒 PRIVÉ PAR DÉFAUT
        ...
        try {
            if (backend !== 'memory') {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    this.botMode = data.botMode || 'private'; // 🔒 PRIVÉ
                    this.messages = data.messages || {};
                    this.isPublic = data.isPublic !== undefined ? data.isPublic : false; // 🔒 PRIVÉ
                    this.cleanupData();
                }
            }
        } catch (e) {
    ...
    async writeToFile(filePath = STORE_FILE) {
        try {
            ...
            const data = {
                ...
                chats: this.chats,
                botMode: this.botMode || 'private', // 🔒 PRIVÉ
        ...
        if (backend === 'memory') {
            try {
                if (fs.existsSync(MESSAGE_COUNT_FILE)) {
                    ...
                    this.isPublic = typeof data.isPublic === 'boolean' ? data.isPublic : false; // 🔒 PRIVÉ
                }
            } catch (e) {
    ...
    async setBotMode(mode) {
        ...
        if (!validModes.includes(mode)) {
            console.warn(`[STORE] Invalid mode: ${mode}, defaulting to private`);
            mode = 'private';
        }
        if (backend === 'memory') {
            this.botMode = mode;
    ...
    // Chatbot Mode Management
    async setChatbotMode(mode) {
        const validModes = ['public', 'private'];
        if (!validModes.includes(mode)) {
            console.warn(`[STORE] Invalid chatbot mode: ${mode}`);
            mode = 'private';
        }
        if (backend === 'memory') {
            ...
            try {
                await adapters[backend].setMetadata('chatbotMode', mode);
            } catch (e) {
                console.error(`[STORE] Failed to set chatbot mode:`, e.message);
            }
        }
    },
    ...
    async getChatbotMode() {
        if (backend === 'memory') {
            ...
            try {
                const mode = await adapters[backend].getMetadata('chatbotMode');
                return mode || 'private';
            } catch (e) {
                console.error(`[STORE] Failed to get chatbot mode:`, e.message);
                return 'private';
            }
        }
    },
