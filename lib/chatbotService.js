...
- If unsure, just chat normally`;
    ...
    async getResponse(userMessage, chatId, senderId, metadata = {}) {
        try {
            ...
            // 🔑 An API key is mandatory — the chatbot won't answer without one configured.
    ...
    _missingApiKeyMessage() {
        return '⚙️ The AI chatbot isn\'t configured yet. Ask the bot owner to run `.cbc apikey <your_key>` (with `.cbc provider <puter|openai|gemini|...>` if needed) to turn it on.';
    }
    ...
    async intelligentCommandDetection(userMessage) {
        try {
            ...
            // Call AI to analyze
            const provider = this.config.provider || 'puter';
            const callProvider = this.providers[provider] || this.providers.puter;
...
RESPOND WITH JSON ONLY. NO OTHER TEXT.`;
    ...
    // === POLLINATIONS (no key of its own, but still gated by the mandatory apiKey check above) ===
    // Open, anonymous, OpenAI-compatible text API. No account of any kind needed for
    // Pollinations itself — kept as an option if you ever relax the key requirement.
    async _callPollinations(userMessage, context, metadata) {
        ...
        try {
            ...
            console.error('Pollinations API request failed:', error.message);
            return this.config.fallbackResponse;
        }
    }

    // === PUTER (free, requires a Puter account token) ===
    // Puter.js exposes an OpenAI-compatible endpoint. Free/unlimited models (like
    // gpt-5.4-nano) don't cost anything to run — you just need a Puter auth token
    // (create a free account and grab it from the Puter dashboard, see
    // https://developer.puter.com/tutorials/puter-auth-token/). Set it with
    // `.cbc apikey <token>` or the PUTER_AUTH_TOKEN env var.
    async _callPuter(userMessage, context, metadata) {
        const authToken = this.config.apiKey || process.env.PUTER_AUTH_TOKEN;
        if (!authToken) {
            console.warn('⚠️ No Puter auth token configured (.cbc apikey <token> or PUTER_AUTH_TOKEN).');
            return this._missingApiKeyMessage();
        }

        const url = 'https://api.puter.com/puterai/openai/v1/chat/completions';
        const model = this.config.puterModel || 'gpt-5.4-nano';
        ...
        try {
            ...
            if (!response.ok) {
                console.error('Puter API error:', await response.text());
                return this.config.fallbackResponse;
            }
            ...
            console.error('Puter API request failed:', error.message);
            return this.config.fallbackResponse;
        }
    }
    ...
    async _callGemini(userMessage, context, metadata) {
        ...
        if (!apiKey) return this._missingApiKeyMessage();
    ...
    async _callNgrok(userMessage, context, metadata) {
        ...
        if (!url) return this._missingApiKeyMessage();
