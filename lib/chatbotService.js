...
- If unsure, just chat normally`;
    ...
    _missingApiKeyMessage() {
        return '⚙️ The AI chatbot isn\'t configured yet. Ask the bot owner to run `.cbc apikey <your_key>` (with `.cbc provider grok` if needed) to turn it on.';
    }
    ...
    async intelligentCommandDetection(userMessage) {
        try {
            ...
            // Call AI to analyze
            const provider = this.config.provider || 'grok';
            const callProvider = this.providers[provider] || this.providers.grok;
...
RESPOND WITH JSON ONLY. NO OTHER TEXT.`;
    ...
    // === GROK (xAI) ===
    async _callGrok(userMessage, context, metadata) {
        const apiKey = this.config.apiKey || process.env.GROK_API_KEY;
        if (!apiKey) {
            console.warn('⚠️ No Grok API key configured (.cbc apikey <token> or GROK_API_KEY).');
            return this._missingApiKeyMessage();
        }

        const url = this.config.apiUrl || 'https://api.x.ai/v1/chat/completions';
        const model = this.config.grokModel || 'grok-1';
