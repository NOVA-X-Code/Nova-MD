...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // ✅ VÉRIFICATION DU MODE IMMÉDIATE
        ...
        // 🔒 Si mode private et pas owner/sudo -> SILENCE TOTAL
        if ((botMode === 'private' || botMode === 'self') && !isOwnerOrSudoCheck) {
            // Ne rien faire du tout - pas de logs, pas de présence, pas d'autoread
            // On retourne immédiatement comme si le bot n'existait pas
            return;
        }
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '*Join our Channel:*\n[https://whatsapp.com/channel/0029Vb8ZJnsAYlUHo1uA6W0y](https://whatsapp.com/channel/0029Vb8ZJnsAYlUHo1uA6W0y)'
                }, { quoted: message });
                return;
            }
...
async function handleGroupParticipantUpdate(sock, update) {
    try {
        ...
        // Vérifier si le bot est en mode private
        const botMode = await store.getBotMode();
                // 🔑 Vérifier si l'auteur est le propriétaire ou sudo
        const isAuthorOwnerOrSudo = author ? await isOwnerOrSudo(author, sock, id) : false;
        const isFromMe = author ? author === sock.user.id || author === `${sock.user.id.split(':')[0]}@s.whatsapp.net` : false;
        const isOwnerOrSudoCheck = isFromMe || isAuthorOwnerOrSudo;
        
        // 🔒 Si mode private/self et l'auteur n'est pas owner/sudo -> SILENCE
        if ((botMode === 'private' || botMode === 'self') && !isOwnerOrSudoCheck) {
            // Ne rien faire, comme si le bot n'existait pas
            return;
        }
        ...
        const isPublicMode = botMode === 'public' || botMode === 'groups' || isOwnerOrSudoCheck;
        switch (action) {
            case 'promote':
                if (!isPublicMode)
                    return;
