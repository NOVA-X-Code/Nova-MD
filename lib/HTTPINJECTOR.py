...
class EHIDecryptor:
    ...
    def execute(cls, file_bytes: bytes) -> Optional[str]:
        ...
        return (
            f"Labokingfreesurf HTTP INJECTOR SCRIPT\n"
            f"{'='*30}\n\n"
            f"{json.dumps(cleaned_final_json, indent=4, ensure_ascii=False)}\n\n"
            f"{'='*30}\n"
            f"code : t.me/Labokingfreesurf and https://whatsapp.com/channel/0029Vb6yLAG9WtC0zbXeEo2t"
        )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if result:
            ...
            print(json.dumps({"error": "Decryption failed"}))
    else:
